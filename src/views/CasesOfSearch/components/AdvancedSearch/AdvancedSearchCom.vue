...
<template>
  ...
  >
    ...
    <div class="search-model">
      <!-- 标题 -->
      <el-row :gutter="10" class="form-title">
        <el-col
          style="width:100px"
      ...
      <div v-for="(item, index) in searchCondition" :key="index">
        ...
        <el-row :gutter="10">
          ...
          <el-col :span="4">
            ...
            >
              <el-option
                v-for="option in getOption(item.parentVariable.dataOptionType)"
                :key="option"
                :label="option"
                :value="option"
              ></el-option>
          ...
          <el-col :span="6">
            ...
            <el-select
              ...
              size="small"
              v-else-if="
                getFormType(item.parentVariable.dataOptionType) === 'select'
              "
            >
              <el-option
                v-for="(listItem, index) in item.parentVariable.dataOption"
                :key="index"
                :label="listItem"
                :value="listItem"
              >
            ...
            <el-date-picker
              ...
              style="width:100%"
              v-else-if="
                getFormType(item.parentVariable.dataOptionType) === 'date' &&
                  item.parentVariable.type !== '区间外' &&
                  item.parentVariable.type !== '区间内'
              "
            >
            </el-date-picker>
            <!-- 区间内 -->
            ...
            <el-row
              v-else-if="
                item.parentVariable.dataOptionType === 1 &&
                  (item.parentVariable.type === '区间外' ||
                    item.parentVariable.type === '区间内')
              "
              style="margin-bottom:0"
          ...
          <el-col :span="4">
            <div class="opr-button">
              ...
              <i
                ...
                style="color:#94dc68 ;cursor:pointer"
                @click="addCondition"
              >
                <!-- 添加 -->
              </i>
              <i
                class="el-icon-error"
                style="color:#d7d7d7;cursor:pointer"
                v-if="index !== 0"
        ...
        <div
          v-if="
            item.advanceSearchVariableDTOList &&
              item.advanceSearchVariableDTOList.length > 0
          "
        >
          <el-row
            :class="i === 0 ? 'first-child' : 'child-row'"
            :gutter="10"
          ...
          >
            ...
            <el-col :span="4">
              ...
              >
                <el-option
                  v-for="option in getOption(child.dataOptionType)"
                  :key="option"
                  :label="option"
                  :value="option"
                ></el-option>
            ...
            <el-col :span="6">
              ...
              >
                <el-option
                  v-for="(listItem, index) in child.dataOption"
                  :key="index"
                  :label="listItem"
                  :value="listItem"
                >
              ...
              <el-date-picker
                ...
                value-format="yyyy-MM-dd HH:mm:ss"
                v-else-if="
                  getFormType(child.dataOptionType) === 'date' &&
                    child.type !== '区间外' &&
                    child.type !== '区间内'
                "
              >
    ...
    <div class="search-footer">
      <el-row :gutter="20">
        <el-col :span="12" class="left-footer">
          筛出患者
        ...
        <el-col :span="12" style="text-align:right">
          <el-button type="text" icon="el-icon-delete" @click="clearCondition"
            >清空条件</el-button
          >
    ...
    <!-- 弹窗 -->
    <el-dialog width="50%" :visible.sync="searchTitVisi">
      <search-pop-com
...
export default {
  ...
  created () {
    this.personNumber = this.advanceSearchListDatas
      ? this.advanceSearchListDatas.personNumber
      : 0 // 患者
    this.visitNumber = this.advanceSearchListDatas
      ? this.advanceSearchListDatas.visitNumber
      : 0 // 病例数
  },
  mounted () {
    if (this.childMianPage) {
      const obj =
        JSON.parse(this.$Storage.localGet('searchJson')) &&
        JSON.parse(this.$Storage.localGet('searchJson'))
          .advanceSearchGroupDTOList
      if (obj) {
        obj.map((item, index) => {
          ...
          if (
            ...
            item.parentVariable.value = [
              item.parentVariable.date1,
              item.parentVariable.date2
            ]
          } else {
            item.parentVariable.value = stringToArr(
              item.parentVariable.value,
              'toString'
            )
          }
  ...
  methods: {
    ...mapMutations([
      'advancedSearch/advancesearchmuta',
      'advancedSearch/advancesearchlistmup'
    ]),
    ...
    searchHandle () {
      ...
      const status = summitData.map(item => {
        if (
          item.parentVariable.name === '' ||
          item.parentVariable.type === '' ||
          item.parentVariable.value.length === 0
        ) {
          return false
        } else {
          return true
    ...
    getHistoryDatas (index, cIndex) {
      ...
      if (this.searchCondition[index].advanceSearchVariableDTOList.length > 0) {
        this.searchCondition[index].advanceSearchVariableDTOList.map(
          childItem => {
      ...
      // console.log(data)
      advancedSearchHistoryDatas(data)
        .then(res => {
    ...
    handleSelect (item) {
      // console.log(item)
      if (this.isChild) {
        this.searchCondition[this.nowIndex].advanceSearchVariableDTOList[
          this.childIndex
        ].id = item.id
        this.searchCondition[this.nowIndex].advanceSearchVariableDTOList[
          this.childIndex
        ...
        this.searchCondition[this.nowIndex].advanceSearchVariableDTOList[
          this.childIndex
        ].dataOptionType = item.dataOptionType
        this.searchCondition[this.nowIndex].advanceSearchVariableDTOList[
          this.childIndex
        ...
        this.searchCondition[this.nowIndex].parentVariable.name =
          item.dataItemName
        this.searchCondition[this.nowIndex].parentVariable.key = item.fieldName
        this.searchCondition[this.nowIndex].parentVariable.dataOptionType =
          item.dataOptionType
        this.searchCondition[this.nowIndex].parentVariable.dataItemCode =
          item.dataItemCode
        // if 类型是4 需要接口获取option
        if (item.dataOptionType === 4) {
          getDataOption(item.dataItemCode).then(res => {
    ...
    dialogDatas (val) {
      ...
      if (val) {
        ...
        if (this.isChild) {
          ...
          this.searchCondition[this.nowIndex].advanceSearchVariableDTOList[
            this.childIndex
          ].name = val.dataItemName
          this.searchCondition[this.nowIndex].advanceSearchVariableDTOList[
            this.childIndex
          ].key = val.fieldName
          this.searchCondition[this.nowIndex].advanceSearchVariableDTOList[
            this.childIndex
          ].dataOptionType = val.dataOptionType
          this.searchCondition[this.nowIndex].advanceSearchVariableDTOList[
            this.childIndex
          ].dataItemCode = val.dataItemCode
          // if 类型是4 需要接口获取option
          ...
          if (val.dataOptionType === 4) {
            getDataOption(val.dataItemCode).then(res => {
              ...
              this.searchCondition[
                this.nowIndex
              ].parentVariable.dataOption = list
            })
          }
        }
          ...
          var results = queryString
            ? restaurants.filter(item => {
              ...
              if (PinyinMatch?.default) {
                newItem =
                    item.value.search(queryString) !== -1 ||
